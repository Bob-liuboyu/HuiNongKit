package com.project.arch_repo.widget.tagview;
...
public class TagView extends RelativeLayout {
    ...
    public TagView(Context context) {
        super(context, null);
        Log.v(TAG, "[TagView]constructor 1");
        init(context, null, 0, 0);
    }

    public TagView(Context context, AttributeSet attrs) {
        super(context, attrs);
        Log.v(TAG,"[TagView]constructor 2");
        init(context, attrs, 0, 0);
    }

    public TagView(Context ctx, AttributeSet attrs, int defStyle) {
        super(ctx, attrs, defStyle);
        Log.v(TAG,"[TagView]constructor 3");
    ...
    public TagView(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        ...
        Log.v(TAG,"[TagView]constructor 4");
    ...
    private void init(Context context, AttributeSet attrs, int defStyle, int defStyleRes) {
        ...
        this.lineMargin = (int) typeArray.getDimension(R.styleable.TagView_lineMargins, ResolutionUtil.dpToPx(this.getContext(), Constants.DEFAULT_LINE_MARGIN));
    ...
    private void drawTags() {
        Log.v(TAG,"[drawTags]visibility = " + (getVisibility() == View.VISIBLE));
        if (getVisibility() != View.VISIBLE) return;
        Log.v(TAG,"[drawTags]mWidth = " + mWidth);
        Log.d(TAG,"[drawTags]add tags, tag count = " + mTags.size());
        // clear all tag
        removeAllViews();
            ...
            tagLayout.setOnLongClickListener(new OnLongClickListener() {
                ...
                public boolean onLongClick(View v) {
                    if(mLongClickListener != null){
                        mLongClickListener.onTagClick(position,tag);
                    }
                    return true;
                }
            });
...
//                        TagView.this.remove(position);
    ...
    public void remove(int position) {
        Log.v(TAG,"[remove]position = " + position);
        mTags.remove(position);
        drawTags();
    }

    public void removeAllTags() {
        Log.v(TAG,"[removeAllTags]");
        mTags.clear();
        drawTags();
    }
    ...
    public OnTagLongClickListener getLongClickListener() {
